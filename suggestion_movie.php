...
<svg class="hidden">
    <symbol id="shoppingcart-svg" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
            stroke-width="1.5" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round"
              d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
    </symbol>
    <symbol id="moon-svg" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round"
              d="M21.752 15.002A9.72 9.72 0 0 1 18 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 0 0 3 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 0 0 9.002-5.998Z" />
    </symbol>
    <symbol id="sun-svg" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round"
              d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z" />
    </symbol>
    <symbol id="exit-svg" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <symbol id="logo-type-svg" viewBox="0 0 138 55" fill="currentcolor" xmlns="http://www.w3.org/2000/svg">
        <path
    ...
    <symbol id="logo-svg" viewBox="0 0 41 40" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
    ...
    <symbol id="phone-arrow-up-right" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
...
<header>
    <div class="w-[90%] rounded-3xl h-24 bg-black/50 backdrop-blur-md mx-auto top-10 right-0 left-0 md:flex justify-between items-center" style="margin-top: 50px">
        ...
        <div class="flex items-center">
            <div class="flex items-center border-l border-gray-300 pl-10">
                <a class="text-gray-300 flex gap-4">
                    <svg class="size-8 toggle-theme">
                        <use xlink:href="#moon-svg"></use>
                    </svg>
                    <svg class="size-8 hidden">
                        <use xlink:href="#sun-svg"></use>
                    </svg>
                </a>
            </div>
            <div class="px-10">
...
<main class="w-full mx-auto flex flex-col items-center" style="margin-top: 75px;">
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-4">
        ...
        <?php foreach ($movies as $movie): ?>
            <div class="w-40 md:w-60 md:h-[80%] bg-sky-950 backdrop-blur-md rounded-lg overflow-hidden shadow-lg flex flex-col pb-2">
                <div class="h-[70%]">
                    <img src="<?= htmlspecialchars($movie['picture']) ?>" alt="<?= htmlspecialchars($movie['title']) ?>" class="w-full h-full">
                </div>
                <div class="h-[30%] p-3 text-white flex flex-col justify-between font-MorabbaMedium text-base/6 leading-relaxed">
                    <span class="text-center pt-5"><?= htmlspecialchars($movie['title']) ?></span>
                    <div class="flex items-center justify-between px-2 text-base font-MorabbaLight text-gray-300">
                <span>
                    <?php
                    $type = $movie['type'];
                    if ($type === 'movie') echo 'فیلم';
                    elseif ($type === 'series') echo 'سریال';
                    elseif ($type === 'anime') echo 'انیمه';
                    else echo htmlspecialchars($type);
                    ?>
                </span>

                        <?php if (isset($_SESSION['user_id'])):
                            $is_liked = in_array($movie['id'], $liked_movie_ids);
                            ?>
                            <a class="heart-wrapper <?= $is_liked ? 'filled' : '' ?>" data-id="<?= htmlspecialchars($movie['id']) ?>" href="javascript:void(0)" aria-label="Like">
                                <svg class="size-6 heart text-red-500 transition-all" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none">
                                    <path d="M12 21C12 21 5 14.5 5 9.5C5 6.5 7.5 4 10.5 4C12 4 13.5 5 14 6.5C14.5 5 16 4 17.5 4C20.5 4 23 6.5 23 9.5C23 14.5 16 21 16 21H12Z"/>
                                </svg>
                            </a>
    ...
    <div class="flex items-center gap-2 justify-center mt-6">
        ...
        if ($page > ($adjacents + 2)) {
            echo '<a href="?page=1" class="w-10 h-10 rounded-full bg-white/10 text-white backdrop-blur-md hover:bg-sky-500 hover:text-black transition-all duration-300 shadow-md flex items-center justify-center">1</a>';
        ...
        for ($i = $start; $i <= $end; $i++) {
            $active_class = ($i == $page) ? 'bg-sky-700 text-white' : 'bg-white/10 text-white';
            echo '<a href="?page=' . $i . '" class="w-10 h-10 rounded-full ' . $active_class . ' backdrop-blur-md hover:bg-sky-500 hover:text-black transition-all duration-300 shadow-md flex items-center justify-center">' . $i . '</a>';
        }

        if ($end < $total_pages - 1) {
            echo '<span class="text-gray-400 px-2">...</span>';
            echo '<a href="?page=' . $total_pages . '" class="w-10 h-10 rounded-full bg-white/10 text-white backdrop-blur-md hover:bg-sky-500 hover:text-black transition-all duration-300 shadow-md flex items-center justify-center">' . $total_pages . '</a>';
